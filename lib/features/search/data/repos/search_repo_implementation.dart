...
class SearchRepoImplementation implements SearchRepo{
  ...
  Future<Either<Failure, List<TopHeadLineModel>>> getSearch({required String query}) async{
    ...
    try{
      var response = await http.get(Uri.parse("${EndPoints.baseUrl}${EndPoints.everything}?q=$query&${EndPoints.language}=${LanguageConsts.currentLanguage}&apiKey=${EndPoints.apiKey}"));
